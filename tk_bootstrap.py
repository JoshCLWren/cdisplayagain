...
prefix. Tk probes for init.tcl relative to the virtualenv, finds nothing, and
every Tk() call fails with "Can't find a usable init.tcl". Pointing the two
library variables at the interpreter's own copies fixes source runs, tests, and
IDE launches alike. Homebrew and system pythons resolve this themselves, so the
directory checks keep this a no-op for them.
"""
